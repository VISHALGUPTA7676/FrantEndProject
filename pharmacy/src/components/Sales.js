...
export const Sales = () => {
  ...
  const handleInputChangePatientName = (event) => {
    ...
    if (value.length == 3) {
      ...
      if ((filterManufacturers(value)) == "") {
        ...
        setpatients(filterManufacturers(value));
      }
    }
  ...
  const handleSelectPatientHand = (patient) => {
  ...
  const filterManufacturers = (input) => {
    return patient.filter(patients =>
      //manufacturer.name.toLowerCase().includes(input.toLowerCase()) && input.length >= 3
      patients.patientName && patients.patientName.toLowerCase().includes(input.toLowerCase()) && input.length >= 3
      //setPurchaseData();
    );
  };
              ...
              {patients.length > 0 && (
                <ul style={{ listStyle: "none", width: "500px" }} className="dropdown">
                  {patients.map(patient => (
                    <li key={patients.id} onClick={() => handleSelectPatientHand(patient)}>
                      {patient.patientName}
